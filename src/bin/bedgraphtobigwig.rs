...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if bedgraphpath == "-" || bedgraphpath == "stdin" {
        ...
        let vals_iter = BedParser::from_bedgraph_stdin();

        let chsi: BedParserStreamingIterator<bigtools::bed::bedparser::BedStdInStream<bigtools::Value>> = BedParserStreamingIterator::new(vals_iter, allow_out_of_order_chroms);
